function nfeq = tide_fac(t_s,t_e,lat,incnstit)
% obj = tide_fac(obj,t_s,t_e,incnstit)   
%       
% Edited by Matthew V Bilskie October 24, 2019,
% Edited to be used in a standalone fashiohn for CE7200 - Fall 2019.
% Copied function from OceanMesh2D, available at:
...
while any(phs < 0)
    ...
    % cnstit.NR.name{k}; % name in frq order
    % cnstit.NR.amp(k); % potential amplitude of species
    % cnstit.NR.frq(k)*2*pi/3600; % frq in rad/s format
    % cnstit.NR.red(k); % earth rigidity reduction factor
    % F(k);   % average nodal factor
    % phs(k); % average nodal correction + astronomical argument at beginning of simulation
    nfeq = cell(3,length(incnstit));
    for k = 1:length(incnstit)
        nfeq{1,k} = cnstit.NR.name{k};  % name of tidal constituent
        nfeq{2,k} = F(k);               % 
        nfeq{3,k} = phs(k);
    end
